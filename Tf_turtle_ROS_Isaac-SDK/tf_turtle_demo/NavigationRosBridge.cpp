...
class CallbackFunctor {
 public:
  ...
  void operator()(const geometry_msgs::Pose::ConstPtr& msg) {
//    const Pose2d pose = Pose2d::FromXYA(msg->x, msg->y, msg->theta);
    LOG_INFO("TF Translation Info from ROS X:%f, Y:%f, Z: %f", msg->position.x, msg->position.y, msg->position.z);
    LOG_INFO("TF Quaternion Info from ROS R:%f, P:%f, Y: %f", msg->orientation.x, msg->orientation.y, msg->orientation.z);
...
void NavigationRosBridge::start() {
  ...
  nav_data_->pub = nav_data_->node.advertise<geometry_msgs::Twist>(
      get_publisher_channel_name(), get_publisher_queue_size());
  nav_data_->sub = nav_data_->node.subscribe<geometry_msgs::Pose>(
